...
class JdbcLinkRepositoryTest extends IntegrationTest {
    ...
    void add_no_error() {
        OffsetDateTime now = OffsetDateTime.now().withOffsetSameLocal(ZoneOffset.UTC).truncatedTo(ChronoUnit.MILLIS);
        Link link = new Link(null, URI.create("https://somesite.com"), now);
        Long id = linkRepository.add(link);

        assertThat(linkRepository.getAll())
            .asList()
            .containsExactly(new Link(id, URI.create("https://somesite.com"), now));
    }
    ...
    void getByUri_no_err() {
        ...
        assertThat(linkRepository.getByUri("https://somesite.com"))
            .isEqualTo(new Link(id, URI.create("https://somesite.com"), now));
    }
    ...
    void updateLastTrackedTime() {
        ...
        OffsetDateTime newTime = OffsetDateTime.MIN;
